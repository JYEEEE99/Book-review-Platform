...
- **대댓글 기능**: 다른 사용자의 댓글에 댓글 달기

### 알림 기능
- **리뷰 알림**: 사용자가 작성한 리뷰에 좋아요/싫어요/댓글 알림
...
#### 기능
- **도서 정보**: 선택한 도서의 제목, 저자, 출판사, 출판일, 도서 미리보기 보임
...
### 도서 추가 및 수정 페이지 (AdminBookForm)
파일: `AdminBookForm.tsx`

#### 기능
- **접근 권한**: 관리자만 페이지에 접근 가능
- **도서 관리**: 도서 추가, 수정, 삭제
...
### 도서 상세 정보
- 도서 제목, 저자, 출판사, 출판일, 도서 미리보기

### 리뷰 목록
- 리뷰 작성: 평점(1~5점) 및 내용 작성
- 리뷰 수정 및 삭제: 작성자 본인만 수정 및 삭제 가능
- 리뷰 평가: 좋아요/싫어요 평가, 평가 수 표시
- 댓글 기능: 다른 사용자의 리뷰에 댓글 달기
...
## 예상 문제점 및 고려 사항
#### 기능 구현의 세부 사항
- 리뷰 작성:  비속어 필터링, 이미지 업로드 등 세부 요구사항이 추가될 수 있음 
...
#### 라우팅되는 주요 페이지 폴더
  - ``Home``: 홈 페이지, 최신 리뷰 섹션, 인기 도서 섹션, 검색 바, 카테고리 필터, 도서 리스트 포함
  - `BookDetail`: 도서 상세 정보 페이지, 도서 정보, 리뷰 리스트, 리뷰 작성 폼 포함
  - ``Signup``: 회원가입 페이지, 이메일, 비밀번호, 닉네임 입력 폼 및 유효성 검사 포함
  - ``Login``: 로그인 페이지, 이메일과 비밀번호로 로그인, 소셜 로그인 버튼 포함
  - ``Profile``: 프로필 페이지, 사용자 정보 보기 및 수정 기능 포함
  - ``Favorites``: 즐겨찾기 페이지, 즐겨찾기에 추가한 도서 및 리뷰 목록 표시
  - ``SearchList``: 사용자가 검색한 키워드에 따라 도서 리스트 보임, 카테고리 필터 기능
  - ``AdminBookForm``: 도서 추가 및 수정 페이지 (관리자 전용)
  - ``ErrorPage404``: 404 Not Found 페이지, 사용자가 존재하지 않는 페이지를 요청했을 때 보여주는 페이지
  - ``ErrorPage500``: 500 Internal Server Error 페이지, 서버에서 예기치 않은 오류가 발생했을 때 사용자에게 알리는 페이지
...
/src
  /app    // 라우팅되는 주요 페이지
    ...
    /favorites
      page.tsx    // Favorites.tsx
    /searchlist
      page.tsx    // SearchList.tsx
    /adminbookform
      page.tsx    // AdminBookForm.tsx
    /404
      page.tsx    // ErrorPage404.tsx
    /500
      page.tsx    // ErrorPage500.tsx
  /components   // 재사용 가능한 UI 컴포넌트 저장
    ...
    /ReviewForm   // 도서 리뷰, 댓글 폼
      ReviewForm.tsx
      ReviewForm.styles.ts
   /features    // 특정 기능과 관련된 상태 관리를 포함한 로직을 관리
    /auth
      authSlice.ts
    /books
      booksSlice.ts
  ...
  /api    // 각 기능별 API 호출을 관리
    /auth
      signupApi.ts
      loginApi.ts
      profileApi.ts
    /books
      booksApi.ts   // 도서 API 통합
      addBookApi.ts
      updateBookApi.ts
      deleteBookApi.ts
    /reviews
